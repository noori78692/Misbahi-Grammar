...
function urduizeDefinition(text: string) {
  ...
  for (const [en, ur] of Object.entries(map)) {
    result = result.replace(new RegExp(`\\b${en}\\b`, "gi"), ur);
  }
  return result;
}
              ...
              <div key={mi}>
                  ...
                  {meaning.definitions.slice(0, 5).map((def, di) => (
                    <div key={di} className="flex gap-3">
                      ...
                      <div className="flex-1">
                        <p className="text-sm text-foreground leading-relaxed" dir="rtl" style={{ textAlign: "right" }}>
                          {urduizeDefinition(def.definition)}
                        </p>
                        {def.example && (
                          <div className="mt-1.5 flex items-start gap-2">
                            <p className="text-xs text-muted-foreground italic flex-1" dir="rtl" style={{ textAlign: "right" }}>
                              “{urduizeDefinition(def.example)}”
                            </p>
                ...
                {meaning.synonyms && meaning.synonyms.length > 0 && (
                  <div className="pt-3 border-t border-border/50">
                    <p className="text-xs font-semibold text-muted-foreground mb-2">{lang === "ur" ? "ہم معنی الفاظ:" : "Synonyms:"}</p>
                    <div className="flex flex-wrap gap-2">
                      {meaning.synonyms.slice(0, 8).map((syn) => (
                        <button key={syn} onClick={() => onSynonymClick(syn)} className="px-2.5 py-1 rounded-lg bg-muted border border-border text-xs font-medium hover:border-primary/50 hover:text-primary transition-colors">
                          {syn}
                        </button>
                      ))}
                    </div>
                ...
                {meaning.antonyms && meaning.antonyms.length > 0 && (
                  <div className="pt-3 border-t border-border/50 mt-3">
                    <p className="text-xs font-semibold text-muted-foreground mb-2">{lang === "ur" ? "متضاد الفاظ:" : "Antonyms:"}</p>
                    <div className="flex flex-wrap gap-2">
                      {meaning.antonyms.slice(0, 6).map((ant) => (
                        <button key={ant} onClick={() => onSynonymClick(ant)} className="px-2.5 py-1 rounded-lg bg-red-50 border border-red-200 text-xs font-medium text-red-700 hover:bg-red-100 transition-colors dark:bg-red-950/30 dark:border-red-800/40 dark:text-red-400">
                          {ant}
                        </button>
                      ))}
                    </div>
  ...
  const handleDictLookup = useCallback(async (wordOverride?: string) => {
    ...
    try {
      if (dictLang === "en") {
        ...
        const cleanEnglish = englishWord.trim().split(/\s+/)[0];
      ...
      <div className="max-w-5xl mx-auto px-4 py-10 space-y-10 flex-1">
        ...
        <section className="rounded-2xl border border-border bg-card overflow-hidden shadow-sm">
          <div className="bg-secondary px-6 py-4 flex items-center gap-3">
            ...
            <div>
              <h2 className="font-serif text-lg font-bold text-secondary-foreground leading-tight">Dictionary</h2>
          ...
          <div className="p-5 md:p-6">
            <div className="flex items-center gap-2 mb-4">
              {(["en", "ur"] as const).map((mode) => (
                <button key={mode} onClick={() => { setDictLang(mode); setDictWord(""); setDictResults([]); setDictError(""); setUrduWordSearched(""); setEnglishEquivalent(""); }} className={`px-4 py-2 rounded-lg text-sm font-semibold border transition-all ${dictLang === mode ? "bg-secondary text-secondary-foreground border-secondary" : "border-border text-foreground/70 hover:bg-muted"}`}>
                  {mode === "en" ? (lang === "ur" ? "انگریزی لفظ" : "English Word") : (lang === "ur" ? "اردو لفظ" : "Urdu Word")}
                </button>
              ))}
            </div>
            <p className="text-sm text-muted-foreground mb-4">{dictLang === "en" ? (lang === "ur" ? "کوئی بھی انگریزی لفظ لکھیں — تعریف، اردو معنی، مثال، ہم معنی اور متضاد الفاظ دیکھیں۔" : "Type any English word to see its definition in Urdu, examples, synonyms, and antonyms.") : (lang === "ur" ? "کوئی بھی اردو لفظ لکھیں — اس کا انگریزی لفظ اور اردو میں مکمل معنی دیکھیں۔" : "Type any Urdu word — see its English equivalent and Urdu meanings.")}</p>
            <div className="flex gap-3 mb-6">
              <div className="relative flex-1">
                <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                <input value={dictWord} onChange={(e) => setDictWord(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") handleDictLookup(); }} placeholder={dictLang === "en" ? (lang === "ur" ? "انگریزی لفظ لکھیں (مثال: beautiful)" : "Type an English word (e.g. beautiful)") : (lang === "ur" ? "اردو لفظ لکھیں (مثال: خوبصورت)" : "Type an Urdu word (e.g. خوبصورت)")} dir={dictLang === "ur" ? "rtl" : "ltr"} className="w-full pl-10 pr-4 py-3 rounded-xl border border-border bg-muted/30 text-sm focus:outline-none focus:ring-2 focus:ring-secondary/40 focus:border-secondary transition-all" />
              </div>
              <button onClick={() => handleDictLookup()} disabled={!dictWord.trim() || dictLoading} className="flex items-center gap-2 px-5 py-3 rounded-xl bg-secondary text-secondary-foreground font-semibold text-sm hover:opacity-90 transition-all disabled:opacity-50 disabled:cursor-not-allowed">{dictLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Search className="w-4 h-4" />}{lang === "ur" ? "تلاش" : "Search"}</button>
            </div>
            {dictError && <div className="p-4 rounded-xl border border-red-200 bg-red-50 text-red-700 text-sm dark:bg-red-950/30 dark:border-red-800/40 dark:text-red-400">{dictError}</div>}
            {dictResults.length > 0 && <DictResults results={dictResults} urduWord={urduWordSearched} englishEquivalent={englishEquivalent} dictLang={dictLang} onSynonymClick={(w) => { setDictWord(w); handleDictLookup(w); }} />}
            {!dictResults.length && !dictLoading && !dictError && <div className="text-center py-8 text-muted-foreground"><BookOpen className="w-10 h-10 mx-auto mb-3 opacity-25" /><p className="text-sm mb-4">{dictLang === "en" ? (lang === "ur" ? "اوپر انگریزی لفظ لکھ کر تلاش کریں" : "Search any English word above") : (lang === "ur" ? "اوپر اردو لفظ لکھ کر تلاش کریں" : "Search any Urdu word above")}</p></div>}
          </div>
        </section>
      </div>
      <footer className="border-t border-border bg-card py-4 text-center text-sm text-muted-foreground mt-auto">
        Developed by Mohammad Noori Misbahi / محمد نوری مصباحی
      </footer>
    </div>
  );
}
