      ...
      <div className="border-b border-border bg-card">
        <div className="max-w-4xl mx-auto px-4 py-10">
          <div className="flex items-center gap-2 text-sm text-muted-foreground mb-3">
            ...
            <Link href="/levels" className="hover:text-primary transition-colors">Course Levels</Link>
            <ChevronRight className="w-3.5 h-3.5" />
            <span className="text-foreground capitalize">{levelId}</span>
          </div>
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h1 className="font-serif text-3xl md:text-4xl font-bold text-primary mb-3 capitalize">{level.title} Grammar</h1>
              <p className="text-muted-foreground text-base max-w-xl">{level.description}</p>
            </div>
          </div>
          ...
          <div className="mt-6 max-w-sm">
            ...
            <div className="h-2.5 rounded-full bg-muted overflow-hidden">
              <div
                className={`h-full rounded-full transition-all duration-500 ${levelBg}`}
      ...
      <div className="max-w-4xl mx-auto px-4 py-10">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    {isNext && (
                      <span className="text-xs px-2 py-0.5 rounded-full bg-primary text-primary-foreground font-medium">Next</span>
                    )}
                    {done && (
                  ...
                  <div className="flex items-center gap-3 mt-2 text-xs text-muted-foreground">
                    <span className="flex items-center gap-1"><BookOpen className="w-3 h-3" /> {lesson.examples.length} examples</span>
                    <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {lesson.quiz.length} quiz questions</span>
                  </div>
                </div>
        ...
        {completed === total && total > 0 && (
          <div className="mt-8 p-6 rounded-2xl border border-emerald-200 bg-emerald-50 text-center">
            ...
            <h3 className="font-serif text-xl font-bold text-emerald-900 mb-2">Level Complete!</h3>
            <p className="text-emerald-700 text-sm mb-4">You have finished all lessons in this level. Test your knowledge with the full level quiz.</p>
            <Link
              href={`/quiz/${levelId}`}
              className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-emerald-700 text-white font-semibold text-sm hover:bg-emerald-800 transition-colors"
            >
              Take Level Quiz
        ...
        <div className="mt-6 text-center">
          <Link href="/levels" className="text-sm text-muted-foreground hover:text-primary transition-colors inline-flex items-center gap-1">
            Back to all levels
          </Link>
        </div>
      </div>
